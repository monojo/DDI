...
static void* mydlsym(void *hand, const char *name)
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "%s = 0x%x\n", name, ret);
    return ret;
}

void dexstuff_resolv_dvm(struct dexstuff_t *d) {
    d->dvm_hand = dlopen("libdvm.so", RTLD_NOW);
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "hand = 0x%x\n", d->dvm_hand);
    if (d->dvm_hand) {
        d->dvm_dalvik_system_DexFile = (DalvikNativeMethod *) mydlsym(d->dvm_hand,
        ...
        d->dvmStringFromCStr_fnPtr = mydlsym(d->dvm_hand,
                                             "_Z32dvmCreateStringFromCstrAndLengthPKcj");
        d->dvmGetSystemClassLoader_fnPtr = mydlsym(d->dvm_hand, "_Z23dvmGetSystemClassLoaderv");
        d->dvmIsClassInitialized_fnPtr = mydlsym(d->dvm_hand,
                                                 "_Z21dvmIsClassInitializedPK11ClassObject");
        ...
        d->dvmFindDirectMethodByDescriptor_fnPtr = mydlsym(d->dvm_hand,
                                                           "_Z31dvmFindDirectMethodByDescriptorPK11ClassObjectPKcS3_");
        ...
        d->dvmCallMethodV_fnPtr = mydlsym(d->dvm_hand,
                                          "_Z14dvmCallMethodVP6ThreadPK6MethodP6ObjectbP6JValueSt9__va_list");
        d->dvmCallMethodA_fnPtr = mydlsym(d->dvm_hand,
                                          "_Z14dvmCallMethodAP6ThreadPK6MethodP6ObjectbP6JValuePK6jvalue");
        d->dvmAddToReferenceTable_fnPtr = mydlsym(d->dvm_hand,
                                                  "_Z22dvmAddToReferenceTableP14ReferenceTableP6Object");

        d->dvmSetNativeFunc_fnPtr = mydlsym(d->dvm_hand,
        ...
        d->dvmDecodeIndirectRef_fnPtr = mydlsym(d->dvm_hand,
                                                "_Z20dvmDecodeIndirectRefP6ThreadP8_jobject");

        d->dvmLinearSetReadWrite_fnPtr = mydlsym(d->dvm_hand,
                                                 "_Z21dvmLinearSetReadWriteP6ObjectPv");

        d->dvmGetCurrentJNIMethod_fnPtr = mydlsym(d->dvm_hand, "_Z22dvmGetCurrentJNIMethodv");

        d->dvmFindInstanceField_fnPtr = mydlsym(d->dvm_hand,
                                                "_Z20dvmFindInstanceFieldPK11ClassObjectPKcS3_");

        //d->dvmCallJNIMethod_fnPtr = mydlsym(d->dvm_hand, "_Z21dvmCheckCallJNIMethodPKjP6JValuePK6MethodP6Thread");
        d->dvmCallJNIMethod_fnPtr = mydlsym(d->dvm_hand,
...
int dexstuff_loaddex(struct dexstuff_t *d, char *path)
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "dexstuff_loaddex, path = 0x%x\n", path);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "cookie = 0x%x\n", pResult.l);
...
void* dexstuff_defineclass(struct dexstuff_t *d, char *name, int cookie)
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "dexstuff_defineclass: %s using %x\n", name, cookie);

    void* cl = d->dvmGetSystemClassLoader_fnPtr();
    Method *m = d->dvmGetCurrentJNIMethod_fnPtr();
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "sys classloader = 0x%x\n", cl);
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "cur m classloader = 0x%x\n", m->clazz->classLoader);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "class = 0x%x\n", ret);
    return ret;
}
