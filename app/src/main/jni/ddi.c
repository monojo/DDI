...
void printString(JNIEnv *env, jobject str, char *l) {
    ...
    if(s) {
        __android_log_print(ANDROID_LOG_DEBUG, "ZX", "%s%s",l, s);
...
static void sb2_connect(JNIEnv *env, jobject obj) {
    printString(env, obj, " sb2 connect");
    __android_log_print(ANDROID_LOG_DEBUG, "ZX", "HERE");
...
static void* sb3_getInputStream(JNIEnv  *env, jobject  obj) {
    ...
    printString(env, obj, " sb3 getInputStream");
...
static void* sb4_getResponseCode(JNIEnv  *env, jobject  obj) {
    ...
    printString(env, obj, " sb4 getResponseCode");
...
static void* sb7_readline(JNIEnv *env, jobject obj) {
    ...
    const char *s = (*env)->GetStringUTFChars(env, res, 0);
   // if (s) {
        //__android_log_print(ANDROID_LOG_DEBUG, "ZX","adb");
        (*env)->ReleaseStringUTFChars(env, res, s);
    //}
    return res;
}
void do_patch() {
   ...
   // dalvik_hook(&d, &sb1);
    ...
    dalvik_hook_setup(&sb2, "Ljavax/net/ssl/HttpsURLConnection;", "connect", "()V", 1, sb2_connect);
    dalvik_hook(&d, &sb2);
